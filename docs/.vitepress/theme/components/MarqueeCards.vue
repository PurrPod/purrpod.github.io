<template>
  <div class="marquee-container">
    <h2 class="marquee-title">{{ heading[localeIndex] }}</h2>
    <div class="marquee-track marquee-track-1">
      <div class="marquee-content">
        <div v-for="item in topFeatures" :key="'1-' + item.title" class="marquee-card">
          <div class="card-icon" v-html="item.icon"></div>
          <span class="card-title">{{ item.title }}</span>
        </div>
        <div v-for="item in topFeatures" :key="'1c-' + item.title" class="marquee-card">
          <div class="card-icon" v-html="item.icon"></div>
          <span class="card-title">{{ item.title }}</span>
        </div>
      </div>
    </div>
    <div class="marquee-track marquee-track-2">
      <div class="marquee-content">
        <div v-for="item in bottomFeatures" :key="'2-' + item.title" class="marquee-card">
          <div class="card-icon" v-html="item.icon"></div>
          <span class="card-title">{{ item.title }}</span>
        </div>
        <div v-for="item in bottomFeatures" :key="'2c-' + item.title" class="marquee-card">
          <div class="card-icon" v-html="item.icon"></div>
          <span class="card-title">{{ item.title }}</span>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup>
import { computed } from 'vue'
import { useData } from 'vitepress'

const { localeIndex } = useData()

const heading = { root: 'Features', en: 'Features' }

const svg = (paths) =>
  `<svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">${paths}</svg>`

const topIcons = [
  svg('<rect x="16" y="16" width="6" height="6" rx="1"/><rect x="2" y="16" width="6" height="6" rx="1"/><rect x="9" y="2" width="6" height="6" rx="1"/><path d="M5 16v-3a1 1 0 0 1 1-1h12a1 1 0 0 1 1 1v3"/><path d="M12 12V8"/>'),
  svg('<rect width="8" height="8" x="3" y="3" rx="2"/><rect width="8" height="8" x="13" y="13" rx="2"/><path d="M7 11v4a2 2 0 0 0 2 2h4"/>'),
  svg('<path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"/>'),
  svg('<path d="M4 14a1 1 0 0 1-.78-1.63l9.9-10.2a.5.5 0 0 1 .86.46l-1.92 6.02A1 1 0 0 0 13 10h7a1 1 0 0 1 .78 1.63l-9.9 10.2a.5.5 0 0 1-.86-.46l1.92-6.02A1 1 0 0 0 11 14z"/>'),
  svg('<circle cx="12" cy="12" r="10"/><path d="M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20"/><path d="M2 12h20"/>'),
  svg('<path d="M19.439 7.85c-.049.322.059.648.289.878l1.568 1.568c.47.47.706 1.087.706 1.704s-.235 1.233-.706 1.704l-1.611 1.611a.98.98 0 0 1-.837.276c-.47-.07-.802-.48-.968-.925a2.501 2.501 0 1 0-3.214 3.214c.446.166.855.497.925.968a.979.979 0 0 1-.276.837l-1.61 1.61a2.404 2.404 0 0 1-1.705.707 2.402 2.402 0 0 1-1.704-.706l-1.568-1.568a1.026 1.026 0 0 0-.877-.29c-.493.074-.84.504-1.02.968a2.5 2.5 0 1 1-3.237-3.237c.464-.18.894-.527.967-1.02a1.026 1.026 0 0 0-.289-.877l-1.568-1.568A2.402 2.402 0 0 1 1.998 12c0-.617.236-1.234.706-1.704L4.23 8.77c.24-.24.581-.353.917-.303.515.077.877.528 1.073 1.01a2.5 2.5 0 1 0 3.259-3.259c-.482-.196-.933-.558-1.01-1.073-.05-.336.062-.676.303-.917l1.525-1.525A2.402 2.402 0 0 1 12 1.998c.617 0 1.234.236 1.704.706l1.568 1.568c.23.23.556.338.877.29.493-.074.84-.504 1.02-.968a2.5 2.5 0 1 1 3.237 3.237c-.464.18-.894.527-.967 1.02Z"/>')
]

const bottomIcons = [
  svg('<line x1="6" x2="6" y1="3" y2="15"/><circle cx="18" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><path d="M18 9a9 9 0 0 1-9 9"/>'),
  svg('<path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2c.71-.84.7-2.13-.09-2.91a2.18 2.18 0 0 0-2.91-.09z"/><path d="m12 15-3-3a22 22 0 0 1 2-3.95A12.88 12.88 0 0 1 22 2c0 2.72-.78 7.5-6 11a22.35 22.35 0 0 1-4 2z"/><path d="M9 12H4s.55-3.03 2-4c1.62-1.08 5 0 5 0"/><path d="M12 15v5s3.03-.55 4-2c1.08-1.62 0-5 0-5"/>'),
  svg('<path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/>'),
  svg('<path d="M16 7h6v6"/><path d="m22 7-8.5 8.5-5-5L2 17"/>'),
  svg('<path d="M22 12h-4l-3 9L9 3l-3 9H2"/>'),
  svg('<path d="m16 18 6-6-6-6"/><path d="m8 6-6 6 6 6"/><path d="m14.5 4-5 16"/>')
]

const titles = {
  top: {
    root: [
      '混合记忆与知识图谱',
      'DAG 可编排工作流',
      '沙盒隔离与本地安全',
      '稳定的 KV 缓存命中率',
      '环境感知与多渠道网关',
      '无代码拓展生态'
    ],
    en: [
      'Hybrid Memory & Knowledge Graph',
      'DAG Orchestrated Workflow',
      'Sandbox Isolation & Local Security',
      'Stable KV Cache Hit Rate',
      'Environmental Perception & Multi-Channel Gateway',
      'No-Code Extension Ecosystem'
    ]
  },
  bottom: {
    root: [
      '会话分支与智能中枢',
      '高并发模型调度',
      '全能原生工具链',
      '自主进化与资产积累',
      '系统心跳与自主长时运行',
      '易于二次开发的工程美学'
    ],
    en: [
      'Session Branching & Agent Hub',
      'High-Concurrency Model Scheduling',
      'All-in-One Native Toolchain',
      'Autonomous Evolution & Asset Accumulation',
      'System Heartbeat & Autonomous Long-Running',
      'Engineering Aesthetics for Secondary Development'
    ]
  }
}

const topFeatures = computed(() =>
  topIcons.map((icon, i) => ({ icon, title: titles.top[localeIndex.value][i] }))
)

const bottomFeatures = computed(() =>
  bottomIcons.map((icon, i) => ({ icon, title: titles.bottom[localeIndex.value][i] }))
)
</script>

<style scoped>
.marquee-container {
  width: 100%;
  overflow: visible;
  padding: 60px 0;
  margin-top: 40px;
  border-top: 1px solid var(--vp-c-divider);
}

.marquee-title {
  text-align: center;
  font-family: var(--vp-font-family-base);
  font-size: 1.5rem;
  font-weight: 600;
  color: var(--vp-c-text-2);
  margin-bottom: 60px;
  letter-spacing: 0.2em;
  text-transform: uppercase;
}

.marquee-track {
  width: 100%;
  overflow: hidden;
  position: relative;
  padding: 30px 0;
  margin-bottom: 10px;
}

.marquee-content {
  display: flex;
  gap: 20px;
  animation: marquee-left 40s linear infinite;
  animation-fill-mode: none;
  animation-timing-function: linear;
  will-change: transform;
  backface-visibility: hidden;
  transform-style: preserve-3d;
  transform: translateZ(0);
}

.marquee-track-2 .marquee-content {
  animation: marquee-right 40s linear infinite;
}

@keyframes marquee-left {
  0% {
    transform: translateX(0) translateZ(0);
  }
  100% {
    transform: translateX(-50%) translateZ(0);
  }
}

@keyframes marquee-right {
  0% {
    transform: translateX(-50%) translateZ(0);
  }
  100% {
    transform: translateX(0) translateZ(0);
  }
}

.marquee-card {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 12px;
  padding: 24px 32px;
  background: var(--vp-c-bg);
  border: 1px solid var(--vp-c-divider);
  border-radius: 16px;
  min-width: 160px;
  transition: all 0.3s ease;
  flex-shrink: 0;
}

.marquee-card:hover {
  border-color: var(--vp-c-brand-2);
  box-shadow: 0 8px 30px rgba(100, 150, 255, 0.2);
  transform: translateY(-6px);
}

.card-icon {
  font-size: 1.75rem;
}

.card-title {
  font-family: var(--vp-font-family-base);
  font-size: 0.95rem;
  font-weight: 500;
  color: var(--vp-c-text-1);
  text-align: center;
  line-height: 1.5;
}

@media (max-width: 768px) {
  .marquee-container {
    padding: 30px 0;
  }

  .marquee-track {
    padding: 12px 0;
  }

  .marquee-content {
    gap: 15px;
    animation-duration: 30s;
  }

  .marquee-track-2 .marquee-content {
    animation-duration: 30s;
  }

  .marquee-card {
    padding: 16px 20px;
    min-width: 120px;
  }

  .card-icon {
    font-size: 1.5rem;
  }

  .card-title {
    font-size: 0.85rem;
  }
}
</style>
