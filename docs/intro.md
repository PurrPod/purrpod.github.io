...
当 Agent 遇到权限不足或缺少关键能力时（如读写宿主机文件、安装 MCP/Skill 插件），向老板发起审批请求。支持文件读写、安装插件、合并技能代码等多种审批类型，是安全防线的最后一道人工闸门。

### 10. BrainStorm 工具：脑暴计划编排

编排复杂项目计划，支持创建具有复杂拓扑结构的 DAG 思维链（Main-Plan + Sub-Branches）。可派发多个后台子分支并发执行，实现"一心多用"，大幅提升多任务并行效率。

### 11. ComputerUse 工具：电脑物理控制

跨平台电脑物理控制工具，让 Agent 像人类一样查看屏幕、移动鼠标、点击按钮和输入文字。支持截图识别 UI 元素 ID 标签、定位点击、拖拽、滚动、键盘快捷键等操作，打通 Agent 与桌面应用的最后一公里。

### 12. KernelUpgrade 工具：自我进化内核

Agent 的自我进化工具。在隔离沙盒中自由创建和升级 Skill / MCP Server，编写测试用例后自动触发后台盲测，通过 evals 验证后可将代码合并至主库。让 Agent 不再被动等待更新，而是主动增长能力。
...
## 九、自主进化：Agent 的自我成长引擎

PurrCat 配备了让 Agent 自己编写自己能力的进化机制。通过 KernelUpgrade 工具和 `src/evolve` 评测流水线，Agent 可以在隔离沙盒中自主创建、升级和测试技能模块，无需等待框架版本更新。

### 技能工厂 (Skill Factory)

当 Agent 在工作中发现缺少某种长期复用的 SOP 工作流，或现存的 Skill 存在 Bug 时，可直接调用 KernelUpgrade 工具在隔离沙盒中进行研发：

- **`create_skill`**：从 0 到 1 搭建全新的技能骨架，包含 SOP 流程、Prompt 模板和 evals 测试用例。
- **`upgrade_skill`**：将表现不佳的现有技能拷贝至沙盒，修复代码逻辑或优化工作流。
- **`test_skill`**：编写完代码和测试用例后，触发后台自动化盲测（Evals），系统会模拟真实场景运行技能并打分。

### MCP 工厂 (MCP Factory)

类似地，Agent 也可以自主创建 MCP 服务器扩展：

- **`create_mcp`**：基于 FastMCP 脚手架搭建新的 MCP Server，自动生成 server.py、config 和 evals。
- **`test_mcp`**：在沙盒中完成开发后触发全链路评测，包括 Schema 校验、工具调用、边界情况测试。

### 评测驱动开发 (Eval-Driven Development)

所有技能的创建和升级都必须经过严格的评测流水线：

1. **编写 Evals**：定义输入、预期输出和评分规则。
2. **触发盲测**：系统在隔离环境运行测试，生成详细 trace 轨迹。
3. **审查报告**：阅读测试日志，定位失败原因并修复。
4. **合并主库**：测试通过后，通过 `Request` 申请 `skill_merge`，将代码合并至主库。
