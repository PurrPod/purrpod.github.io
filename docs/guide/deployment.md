...
- **Miniconda 或 Anaconda**：用于管理和隔离 Python 虚拟环境。
...
Agent 需要调用大语言模型（LLM）和视觉语言模型（VLM）来处理任务。您需要在这个文件中填入您的 API 凭据。请打开 `data/config/secrets/models.yaml` 文件：

- **大语言模型 (LLM)**：默认配置标识为 `"openai:deepseek-chat"`。请找到 `api_keys` 节点，将 `"api-key-1"` 等占位符替换为您申请的真实 API Key。

- **视觉语言模型 (VLM)**：默认配置标识为 `"openai:qwen3-vl-plus"`。请同样将其 `api_keys` 下的占位符替换为真实的 API Key。

- **扩展专用模型**：如果您希望启用图像生成、音频处理或视频转换等高级功能，可在同一文件的 `specialized_models` 配置块中填入对应的 API Key 和 Base URL。

注意：目前 CatInCup 仅支持可通过 OpenAI SDK 调用的模型。
...
启动脚本在运行时会自动激活 `CatInCup` 的 conda 环境，启动由 `backend.py` 驱动的后台服务，并同时启动 Next.js 的前端服务 (`npm run dev`)。服务成功挂载后，您可以根据终端中的提示访问本地地址。如果您需要关闭应用，在终端按下 `Ctrl+C` 即可安全终止所有关联进程。